...
test("planning lock: UI holding the lock blocks Agent plan mutation and returns a clear message", async () => {
  ...
  const res = await tool.execute("tc1", { request: { action: "plan_add", start: "a0001", end: "a0002" } }, null, null, toolCtx);
  ...
  const show = await tool.execute("tc2", { request: { action: "plan_show" } }, null, null, toolCtx);
...
test("planning lock: reload does not restore the old lock but DraftPlan is restored", async () => {
  ...
  const tool = await startAgentFirst(pi, toolCtx, commandCtx);
  await tool.execute("tc1", { request: { action: "plan_add", start: "a0001", end: "a0002" } }, null, null, toolCtx);
  await tool.execute("tc2", { request: { action: "plan_update", range_id: "d1", summary: "filled." } }, null, null, toolCtx);
  ...
  const show = await tool.execute("tc3", { request: { action: "plan_show" } }, null, null, freshCtx);
